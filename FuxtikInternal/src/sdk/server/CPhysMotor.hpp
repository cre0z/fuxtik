...
namespace source2sdk::server
{
    ...
    public:
        ...
        char m_hAttachedObject[0x4]; // 0x4c8        
        float m_spinUp; // 0x4cc        
        float m_additionalAcceleration; // 0x4d0        
        float m_angularAcceleration; // 0x4d4        
        client::GameTime_t m_lastTime; // 0x4d8        
        [[maybe_unused]] std::uint8_t pad_0x4dc[0x14]; // 0x4dc
        server::CMotorController m_motor; // 0x4f0        
    ...
    static_assert(sizeof(CPhysMotor) == 0x510);
};
